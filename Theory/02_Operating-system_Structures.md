...
- **프로그램 실행**: 시스템은 **프로그램을 메모리에 로드하고 실행**할 수 있어야 하며, 정상적이든 비정상적이든 실행을 종료하는 기능을 제공한다.
	- 정상적 종료: 모든 프로그램을 수행 후 종료.
	- 비정상적 종료: 내부적으로 프로세스를 실행하다가 에러 발생하여 종료, 외부적인 이유로 강제로 종료 시
    
- **입출력 작업**: 실행 중인 프로그램은 **파일이나 입출력 장치와 관련된 작업**이 필요할 수 있으며, 운영체제는 이를 제어한다.
    
- **파일 시스템 조작 (File-system Manipulation)**: 프로그램은 **파일을 읽고 쓰고, 디렉토리를 생성하거나 삭제하며, 권한 관리 및 정보 열람 등의 작업**을 수행한다.
    
- **통신**: 동일 컴퓨터 내의 프로세스 간 또는 네트워크로 연결된 다른 컴퓨터의 **프로세스 간에 정보를 교환하는 기능**을 제공한다. 이는 공유 메모리(Shared Memory)나 메시지 전달(Message Passing) 방식을 통해 이루어진다.
    
- **오류 탐지**: 하드웨어(CPU, 메모리, 입출력 장치)나 사용자 프로그램에서 발생할 수 있는 잠재적인 **오류를 끊임없이 감지**하고 적절한 조치를 취해 **계산의 일관성**을 유지한다. 또한, **디버깅 기능**은 사용자와 프로그래머가 시스템을 효율적으로 사용하는 능력을 크게 향상시킬 수 있다.
    

### 2. **시스템 효율성**을 위한 서비스

- **자원 할당**: **여러 사용자나 작업이 동시에 실행**될 때 CPU 사이클, 메인 메모리, 파일 저장소 등의 **자원을 각 작업에 적절히 배분**한다.
    
- **계정**: 어떤 사용자가 어떤 종류의 자원을 얼마나 사용하는지 **추적하고 기록**한다.
    
- **보호 및 보안**: 다중 사용자 시스템에서 정보에 대한 접근을 제어하고, 외부의 부적절한 접근으로부터 시스템을 방어하며 사용자 인증을 수행한다.
...
- **System-call Interface**는 이 번호에 따라 인덱싱된 테이블을 유지 관리한다.
...
- 시간 대기 및 이벤트 신호 대기/송신.
    
- 메모리 할당 및 해제.
    
- 오류 발생 시 메모리 상태를 덤프 파일로 저장
...
- **파일 관리**: 파일 생성, 삭제, 열기, 닫기, 읽기, 쓰기, 위치 재조정 등을 수행한다.
    
- **장치 관리**: 장치 요청, 해제, 읽기, 쓰기 작업을 수행하며, 장치를 논리적으로 부착하거나 분리한다.
...
- 자원에 대한 접근 제어 및 권한 설정을 통해 사용자 접근을 허용하거나 거부한다.
...
- **파일 관리**: 파일과 디렉터리를 생성, 삭제, 복사, 이름 변경, 인쇄, 덤프, 목록 표시 및 조작하는 기능을 담당한다.
    
- **상태 정보**: 날짜, 시간, 가용 메모리 및 디스크 공간량, 사용자 수 등 시스템 정보를 요청하여 제공한다. 일부 시스템은 구성 정보를 저장하고 검색하기 위해 **Registry**를 구현하여 사용한다.
	- **Registry:** 시스템 설정정보나 HW 및 SW의 구성 데이터를 통합해서 관리하는 거대한 데이터 베이스.
    
- **파일 수정**: 파일을 생성하고 수정하기 위한 텍스트 에디터나 파일 내용 검색 및 텍스트 변환을 위한 특수 명령어를 포함한다.
    
- **프로그래밍 언어 지원**: 컴파일러, 어셈블러, 디버거 및 인터프리터가 제공된다.
    
- **프로그램 로딩 및 실행**: Absolute loaders, Relocatable loaders, Linkage editors 및 디버깅 시스템을 제공한다.
...
- **통신**: 프로세스, 사용자 및 컴퓨터 시스템 간의 가상 연결을 생성하는 메커니즘을 제공한다. 사용자가 서로 메시지를 보내거나 웹 페이지 브라우징, 전자우편 전송, 원격 로그인, 파일 전송 등을 가능하게 한다.
...
## 운영체제 구현 (Implementation)

운영체제를 작성하는 언어와 방식은 시스템의 성능과 개발 효율성에 직접적인 영향을 미친다. 과거와 현재의 기술적 변화를 통해 운영체제 구현의 특성을 이해할 수 있다.

- **초기 운영체제**: 초기에는 하드웨어 자원을 직접 제어하여 성능을 극대화하기 위해 **어셈블리어**로 작성되었다.
    
- **현대의 시스템 프로그래밍**: 현재는 **C**나 **C++** 같은 고급 언어를 주로 사용한다. 실제로는 여러 언어가 혼합되어 사용되는데, 하드웨어와 직접 맞닿는 최하위 수준은 어셈블리어로, 운영체제의 본체는 C로, 시스템 프로그램은 C, C++ 및 **펄(PERL)**, **파이썬(Python)**, **셸 스크립트(Shell scripts)** 같은 스크립트 언어로 작성된다.
    
- **고급 언어 사용의 이점**: 고급 언어를 사용하면 다른 하드웨어로 운영체제를 옮기는 **Porting**이 훨씬 쉬워진다. 다만, 어셈블리어에 비해 실행 속도가 다소 느려질 수 있다는 단점이 있다.
    
- **에뮬레이션(Emulation)**: 에뮬레이션 기술을 활용하면 운영체제가 원래 설계되지 않은 Non-native 하드웨어 위에서도 실행될 수 있다.

## 운영체제 구조

범용 운영체제는 매우 거대한 프로그램이므로 이를 체계적으로 관리하기 위한 다양한 구조적 접근 방식이 존재한다.

- **구조의 종류**: 운영체제의 크기와 목적에 따라 여러 방식이 존재한다. 
	- **단순 구조(Simple structure)**: MS-DOS
	- **복잡한 구조(More complex)**: UNIX
	- **계층적 구조(Layered approach)**: 추상화
	- **마이크로커널(Microkernel)**: Mach

## 단순 구조: MS-DOS 

최소한의 공간에서 최대한의 기능을 제공하기 위해 설계된 초기 운영체제의 전형적인 모습이다.

![](../images/Pasted_image_20260324144317.png)
애플리케이션 프로그램이 상주 시스템 프로그램이나 장치 드라이버를 거치지 않고 직접 ROM BIOS 장치 드라이버에 접근할 수 있다. -> layered 구조라면 중간 단계를 스킵하고 넘어갈 수 없다.

- **설계 특징**: 모듈화가 제대로 이루어지지 않았으며, 인터페이스와 기능의 계층이 명확하게 분리되어 있지 않다. 
    
- **성능과 위험성**: 계층 간의 구분이 없어서 속도는 빠를 수 있으나, 사용자 프로그램이 기본 입출력 시스템(BIOS)에 직접 접근할 수 있어 시스템 전체의 안정성이 취약하다는 단점이 있다.

## Non simple structure - 전통적인 유닉스 시스템 구조 

유닉스는 하드웨어의 제한으로 인해 초기에는 **완전한 계층 구조를 갖추지 못했으나,** 크게 두 부분으로 분리된 구조를 가진다.

- **시스템 프로그램(Systems programs)**: 사용자와 운영체제 간의 인터페이스 역할을 수행한다.
    
- **커널(Kernel)**: 시스템 호출 인터페이스(System-call interface) 아래부터 하드웨어 인터페이스 위의 모든 기능을 포함한다. **파일 시스템**, **CPU 스케줄링**, **메모리 관리** 등 방대한 기능을 하나의 층(Level)에서 처리한다.
    

![](../images/Pasted_image_20260324144552.png)
사용자가 사용하는 셸과 명령어가 시스템 호출을 통해 커널 내부의 신호 처리, 단말기 드라이버, 파일 시스템 등에 접근하고, 다시 커널이 하드웨어를 제어하는 복잡한 연결 관계를 나타낸다.

## 계층적 접근 방식 (Layered Approach)

운영체제를 여러 개의 층(Layer)으로 나누어 상위 층이 하위 층의 서비스만을 이용하도록 설계하는 추상화 방식이다.

![](../images/Pasted_image_20260324144850.png)
중심부의 하드웨어를 시작으로 여러 층이 감싸고 있으며, 가장 바깥쪽에 사용자 인터페이스가 위치한 동심원 형태의 도표다.

- **계층의 구성**: 최하위 층인 **Layer 0**은 하드웨어이며, 최상위 층인 **Layer N**은 사용자 인터페이스(User interface)다.
    
- **모듈성**의 이점: 각 층은 **자신의 바로 아래 층에서 제공하는 기능과 서비스만을 사용**하도록 선택된다. 이로 인해 시스템의 설계와 디버깅이 매우 단순해진다. 특정 층에서 오류가 발생하면 그 아래 층들은 이미 검증된 상태이므로 해당 층만 조사하면 되기 때문이다.